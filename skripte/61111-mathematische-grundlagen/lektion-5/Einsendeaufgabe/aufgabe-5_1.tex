\documentclass[a4paper,12pt]{article}
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage[ngerman]{babel}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{enumitem}
\usepackage{geometry}
\geometry{a4paper, margin=2.5cm}

\title{Einsendeaufgaben -- Lektion 5\\[0.5em]
\large Modul 61111: Mathematische Grundlagen}
\author{}
\date{}

\begin{document}
\maketitle

\section*{Aufgabe 5.1}

Sei $x \in \mathbb{R}$ beliebig.

\underline{Fall 1: $x < -1$}

Sei $(x_n)$ eine beliebige Folge mit Grenzwert $x$.
Wir definieren ein $\varepsilon := \frac{|x - (-1)|}{2}$.

Es gilt dann $x < x + \varepsilon < -1$ und für fast alle $x_n$ gilt $x_n < x + \varepsilon < -1$.
Und daher für fast alle $x_n$ gilt:
\[
    f(x_n) = x_n + 2.
\]

Da $x + 2$ stetig auf ganz $\mathbb{R}$ ist, konvergiert $(f(x_n))$ gegen $f(x)$.

\underline{Fall 2: $x > -1$}

Gleiche Argumentation wie Fall 1.

\underline{Fall 3: $x = -1$}

Wir verwenden das $\varepsilon$-$\delta$-Kriterium.
Sei $\varepsilon > 0$ beliebig und $(x_n)$ eine Folge mit Grenzwert $-1$.

Da jede Polynomfunktion stetig ist, existiert ein $\delta_1 > 0$, sodass
für alle $x_n$ mit $|x_n + 1| < \delta_1$ gilt:
\[
    |(x_n + 2) - (x + 2)| < \varepsilon.
\]

Genauso existiert ein $\delta_2 > 0$, sodass
für alle $x_n$ mit $|x_n + 1| < \delta_2$ gilt:
\[
    |x_n^2 - x^2| < \varepsilon.
\]

Wir definieren ein $\delta := \min(\delta_1, \delta_2)$.

Zu zeigen ist nun: für alle $x_n$ mit $|x_n + 1| < \delta$ gilt:
\[
    |f(x_n) - f(x)| < \varepsilon.
\]

\underline{Fall 3.1: $x_n \leq -1$}

Da $\delta \leq \delta_1$ gilt, folgt $|x_n + 1| < \delta \leq \delta_1$.
Wegen $x_n \leq -1$ gilt $f(x_n) = x_n + 2$.
Aus $|x_n + 1| < \delta_1$ und $f(x_n) = x_n + 2$ folgt:
\[
    |f(x_n) - f(x)| = |(x_n + 2) - (x + 2)| < \varepsilon.
\]

\underline{Fall 3.2: $x_n > -1$}

Gleiche Argumentation.

\end{document}
