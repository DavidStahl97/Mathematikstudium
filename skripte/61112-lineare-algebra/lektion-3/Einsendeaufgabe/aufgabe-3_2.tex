...
Sei $\beta \in \mathrm{Bil}_K(V)$ beliebig. Es existiert eine geordnete Basis $B = (v_1, \ldots, v_n)$.

\bigskip
\textbf{\glqq$\Rightarrow$\grqq-Richtung}

Es gelte $\mathrm{Rg}(\beta) = 1$.
Nach Satz 3.1.19 gilt
\[
  \beta(u,v) = k_B(u)^T M_B(\beta)\, k_B(v).
\]

Da $\mathrm{Rg}(\beta) = 1$, also auch $\mathrm{Rg}(M_B(\beta)) = 1$, gilt für $f: V \to K^n,\ v \mapsto M_B(\beta)\, k_B(v)$:
\[
  \mathrm{Rg}(f) = 1.
\]
...
\bigskip
\textbf{\glqq$\Leftarrow$\grqq-Richtung}
